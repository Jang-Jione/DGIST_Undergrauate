...
X_test = X_test.transpose(0, 2, 3, 1)  # (100, 1, 28, 28) -> (100, 28, 28, 1) -> keras에서는 이 형식을 요구한다고 한다.
...
model = keras.Sequential([
    keras.layers.Conv2D(filters=16, kernel_size=(3, 3), strides=(1, 1), padding = "same", input_shape=(28, 28, 1)),
    keras.layers.Activation('relu'),
    keras.layers.MaxPool2D(pool_size=(2,2), strides=2),
    keras.layers.Conv2D(filters=16, kernel_size=(3, 3), strides=(1, 1), padding = "same", input_shape=(28, 28, 1)),
    keras.layers.Activation('relu'),
    keras.layers.MaxPool2D(pool_size=(2,2), strides=2),
    keras.layers.Flatten(), # 차원 변환하고 해야해!!
