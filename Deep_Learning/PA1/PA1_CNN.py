...
# 이 코드 자체는 PA1_NN과 거의 동일한 코드!
########## 데이터 로드

dataloader_train = dataloader.Dataloader('G:/mnist/dataset', is_train=True, shuffle=True, batch_size=100)
dataloader_test = dataloader.Dataloader('G:/mnist/dataset', is_train=False, shuffle=True, batch_size=100)
...
# reshaping: train은 batch 부분에서 해준다. -> CNN에서는 input size 그대로 가져가야 한다.
...
########## initializing
# 모델 정의
model = CNN.ConvLayerNN()

# cross_entropy 정의
softmax_loss = CNN.Softmax_loss()
...
for epoch in range(epochs):
    ...
    for batch_idx, batch in enumerate(dataloader_train):
        ...
        if (batch_idx == 100):
            break # test 코드 -> 잘 돌아간다.. 근데 너무 오래 걸린다..
        # 배치 단위로 데이터 가져오기
        # dataloader_train.batch_size = 8(default 값)
        X_batch, Y_batch = batch

        # input을 1-dim vector로 변경 -> CNN에서는 input size 그대로 가져가야 한다.**
        ...
        train_prediction = CNN.softmax(train_output) 
        ...
        print(train_loss_at_iter/dataloader_train.batch_size)
        ...
        # test_prediction = CNN.softmax(test_output) 
        ...
        ########## test에서도 한 배치만 사용하기 -> 여기선 다 돌리면 forward 연산 너무 오래 걸린다.
        
        X_test_batch, Y_test_batch = dataloader_test[batch_idx%len(dataloader_test)]
        test_output = model.forward(X_test_batch)
        test_prediction = CNN.softmax(test_output) 
        test_loss += softmax_loss.forward(test_output, Y_test_batch) # epoch에서 평가를 위한 코드
        test_loss_at_iter = softmax_loss.forward(test_output, Y_test_batch) # figure에서 평가를 위한 코드
        test_correct += np.sum(np.argmax(test_prediction, axis=1) == np.argmax(Y_test_batch, axis=1))
        
        test_loss_at_iter_list.append(test_loss_at_iter/(dataloader_test.batch_size))
