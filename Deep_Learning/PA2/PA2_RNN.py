...
########## 초기화
# 모델 정의
model = RNN.RNN(input_size=50,  
            hidden_size=128,
            output_size=5,
            learning_rate=0.001,
            glove_file='G:/emo/embedding/glove.6B.50d.txt',
            dropout_rate=0.0,
            optimizer="SGD") # rate에 따라서 학습이 안되기도 한다. 이거 잘 조절하기!
# rnn sgd rate 0.001
# rnn adam rate 0.00001

# cross_entropy 정의
softmax_loss = RNN.Softmax_loss()
...
for epoch in range(epochs):
    ...
    for batch_idx in range(num_batches):
        ...
        train_prediction = RNN.softmax(train_output) 
...
test_prediction = RNN.softmax(test_output)
